...
const EXCHANGE_RATE = 1570; 

const RAW_PRODUCTS = [
  { 
    id: 'chatgpt_shared', 
    name: 'ChatGPT Plus (پرۆفایلێ بەشکرن)', 
    priceUSD: 10, 
    originalPriceUSD: 20, 
    category: 'هاریکارێ AI', 
    badge: 'داشکاندن ٥٠٪ 🔥' 
  },
  { 
    id: 'chatgpt_private', 
    name: 'ChatGPT Plus (ئەکاونتێ تایبەت)', 
    priceUSD: 20, 
    originalPriceUSD: 25, 
    category: 'هاریکارێ AI', 
    badge: 'فەرمی / تایبەت' 
  ...
  { 
    ...
    priceUSD: 4, 
    originalPriceUSD: 8, 
    category: 'فلم و زنجیرە', 
    badge: 'پرۆفایلێ تایبەت' 
  },
  { 
    id: 'elevenlabs', 
    name: 'ElevenLabs AI', 
  ...
  { 
    id: 'bundle_ai', 
    name: 'پاکێجا AI VIP (ChatGPT + Claude)', 
    priceUSD: 36, 
    originalPriceUSD: 45, 
  ...
  { 
    id: 'bundle_creator', 
    name: 'پاکێجا دروستکەران (Canva + ElevenLabs)', 
    priceUSD: 18, 
    originalPriceUSD: 28, 
    category: 'ئۆفەرێ تایبەت', 
    badge: 'داشکاندن 🔥', 
...
const PRODUCTS = RAW_PRODUCTS.map(item => ({
  ...
  originalPriceIQD: item.originalPriceUSD ? Math.round(item.originalPriceUSD * EXCHANGE_RATE) : null
}));
              ...
              return (
                ...
                >
                  <div>
                    ...
                    <div className="mb-6">
                      {product.originalPriceIQD && (
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-xs text-rose-400/80 line-through font-medium">
                            {product.originalPriceIQD.toLocaleString()} IQD
                          </span>
                    ...
                    {cart.map((item) => (
                      <div key={item.id} className="flex justify-between items-center bg-slate-950/70 p-3 rounded-xl border border-slate-800 text-xs">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button type="button" onClick={() => removeFromCart(item.id)} className="text-slate-500 hover:text-rose-400 transition-colors">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      </div>
