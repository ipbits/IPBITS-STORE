...
const DEFAULT_MODELS = [
  ...
  // مۆدێلێن چات و شیکاریا وێنەیان و کۆدینگێ
...
export default function ChatPage() {
  ...
  // ئینانا هەمی مۆدێلێن OpenRouter ب ئۆتۆماتیکی
  useEffect(() => {
  ...
  // خاندنا ناڤەرۆکا فایلی کۆدی یان تێکستێ و زێدەکرن بۆ ناڤ Input
  ...
  return (
    <div className="min-h-screen bg-[#070913] text-white flex flex-col justify-between font-sans" dir="rtl">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 max-w-4xl w-full mx-auto">
        {messages.map((m, idx) => (
          ...
          >
            ...
            >
              {/* پیشاندانا وێنەیێ بارکری ژ لایێ بەکارهێنەری */}
              {m.image && (
                <img 
                  src={m.image} 
                  alt="Uploaded Asset" 
      ...
      {/* جهێ نڤیسینێ و بارکرنا وێنە و فایلی کۆدی */}
      <div className="p-4 border-t border-slate-800/80 bg-slate-950/60 backdrop-blur-md">
        ...
        <form onSubmit={handleSend} className="max-w-4xl mx-auto flex items-center gap-2">
          ...
          {/* دوگمەیا بارکرنا فایلێن کۆد و تێکستێ (Paperclip) */}
          <input
            type="file"
          ...
          <button
            ...
            title="بارکرنا فایلی کۆدی یان تێکست"
          >
            <Paperclip size={18} />
          </button>

          {/* دوگمەیا بارکرنا وێنەی (Image) */}
          <input
            type="file"
            accept="image/*"
            ref={fileInputRef}
          ...
          <input
            ...
            placeholder="پرسیارا خۆ بنڤیسە، وێنەیەکێ بار بکە یان فایلی کودی بەست بدە..."
